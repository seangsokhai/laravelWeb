        ...
        <form method="POST" action="{{ route("admin.student.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group col-md-6">
                    ...
                    <select class="form-control select2 {{ $errors->has('course_id') ? 'is-invalid' : '' }}" name="course_id" id="course_id" required>
                        @foreach($course as $id => $c)
                            <option value="{{ $id }}" {{ old('course_id') == $id ? 'selected' : '' }}>{{ $c }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-6">
                    ...
                    <select class="form-control select2 {{ $errors->has('schedule_id') ? 'is-invalid' : '' }}" name="schedule_id" id="schedule_id">
                        @foreach($schedule as $id => $i)
                            <option value="{{ $id }}" {{ old('schedule_id') == $id ? 'selected' : '' }}>{{ $i }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    <input class="form-control {{ $errors->has('company_or_school') ? 'is-invalid' : '' }}" type="text" name="company_or_school" id="company_or_school" value="{{ old('company_or_school') }}" step="1" required>
                ...
                <div class="form-group col-md-6">
                    ...
                    <input class="form-control {{ $errors->has('position_or_subject') ? 'is-invalid' : '' }}" type="text" name="position_or_subject" id="position_or_subject" value="{{ old('position_or_subject') }}" step="1" required>
